//! Video writer — encode a sequence of frames into a video file.
//!
//! This module provides [`VideoWriter`] for encoding `DynamicImage` frames into
//! a video container (MP4, MKV, AVI, etc.) using FFmpeg.
//!
//! # Example
//!
//! ```no_run
//! use unbundle::{MediaUnbundler, FrameRange, VideoWriter, VideoWriterConfig};
//!
//! let mut unbundler = MediaUnbundler::open("input.mp4")?;
//! let frames = unbundler.video().frames(FrameRange::Range(0, 10))?;
//! VideoWriter::new(VideoWriterConfig::default())
...
///
/// Create via [`VideoWriter::new`], then call [`write`](VideoWriter::write).
pub struct VideoWriter {
    config: VideoWriterConfig,
}

impl VideoWriter {
    /// Create a new video writer with the given configuration.
    pub fn new(config: VideoWriterConfig) -> Self {
        Self { config }
    }
