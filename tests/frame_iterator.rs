...
#[test]
fn frame_iter_with_config_pixel_format() {
    let path = sample_video_path();
    if !Path::new(path).exists() {
        return;
    }
    ...
    let iter = unbundler
        .video()
        .frame_iter_with_config(FrameRange::Range(0, 0), config)
        .expect("Failed to create iterator");
...
fn frame_iter_matches_frames_count() {
    ...
    let mut unbundler2 = MediaUnbundler::open(path).expect("Failed to open fixture");
    let iter = unbundler2
        .video()
        .frame_iter(range)
        .expect("Failed to create iterator");
