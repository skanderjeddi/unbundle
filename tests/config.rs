...
fn config_defaults() {
    ...
    assert!(debug.contains("ExtractionConfig"));
...
fn config_with_batch_size() {
    let config = ExtractionConfig::new().with_batch_size(10);
    let debug = format!("{config:?}");
...
fn config_with_batch_size_clamps_zero() {
    let config = ExtractionConfig::new().with_batch_size(0);
    let debug = format!("{config:?}");
    // Clamped to 1.
    assert!(debug.contains("batch_size: 1"));
}
...
// ── FrameOutputConfig resolve_dimensions ───────────────────────────
