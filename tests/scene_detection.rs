//! Integration tests for scene detection (feature = "scene-detection").
//!
//! These tests require the `scene-detection` feature and test fixtures
...
fn detect_scenes_scene_change_fields() {
    ...
    let scenes = unbundler
        .video()
        .detect_scenes(Some(SceneDetectionConfig { threshold: 1.0 }))
        .unwrap();
...
fn detect_scenes_no_video_stream_error() {
    ...
    let mut unbundler = MediaUnbundler::open(AUDIO_ONLY).unwrap();
...
fn detect_scenes_with_cancellation() {
    ...
    use unbundle::{CancellationToken, ExtractionConfig};

    let mut unbundler = MediaUnbundler::open(SAMPLE_VIDEO).unwrap();
    let token = CancellationToken::new();
    token.cancel();

    let config = ExtractionConfig::new().with_cancellation(token);
    let result = unbundler
        .video()
        .detect_scenes_with_config(None, &config);
...
fn detect_scenes_mkv_format() {
    ...
    let mut unbundler = MediaUnbundler::open(SAMPLE_MKV).unwrap();
    let scenes = unbundler.video().detect_scenes(None).unwrap();
...
#[test]
fn scene_detection_config_debug() {
    let config = SceneDetectionConfig { threshold: 42.0 };
