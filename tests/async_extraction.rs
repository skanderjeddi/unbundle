//! Async frame-stream integration tests (feature = "async-tokio").
//!
//! These tests require the `async-tokio` feature and test fixtures
...
use unbundle::{ExtractionConfig, FrameRange, FrameStream, MediaUnbundler};
...
async fn stream_range() {
    ...
    let mut stream = unbundler
        .video()
        .frame_stream(FrameRange::Range(10, 14), ExtractionConfig::new())
        .unwrap();
...
async fn stream_no_video_stream_error() {
    ...
    let result = unbundler
        .video()
        .frame_stream(FrameRange::Range(0, 0), ExtractionConfig::new());
...
async fn audio_future_extracts_wav() {
    ...
    let audio_bytes = unbundler
        .audio()
        .extract_async(unbundle::AudioFormat::Wav, ExtractionConfig::new())
        .unwrap()
        .await
        .unwrap();
